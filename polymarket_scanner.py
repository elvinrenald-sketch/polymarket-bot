#!/usr/bin/env python3
"""
POLYMARKET FULL AUTO BOT v11.1
================================
FIXES:
...
# ── FIX: Set TERM sebelum import colorama ──────────────────────────
import os, sys
...
def analyze(names, gamma_px, clob, liq, vol, days, prev_px) -> Optional[dict]:
    ...
    # ── Tentukan sinyal ─────────────────────────────────────────
    is_strong = False
    is_auto   = False  # Hanya STRONG BUY & ARBITRAGE yang auto-open

    if is_arb and arb_profit > 0.2:
        signal = 'ARBITRAGE'; action = f'BELI ALL'; color = GG
        ...
        entry_name = d; is_strong = True  # tampil, tapi is_auto=False
        ...
        signal = 'EDGE'; action = f'WATCH'; color = YY
    ...
    # Cari entry_idx sesuai entry_name
    ...
    return {
        ...
        'score'         : score,
        'is_strong'     : is_strong,
        'is_auto'       : is_auto,   # BARU: flag untuk auto-open
        'clob'          : clob,
    }
...
# ══════════════════════════════════════════════════════════════════
# POSITION MANAGER — Auto Open & Auto Close
# ══════════════════════════════════════════════════════════════════
class PositionManager:
    ...
    async def check_and_close(self, session) -> List[dict]:
        ...
        for pos in self.open_positions:
            ...
            # ── Cek apakah perlu close ──────────────────────────
            should_close = False
            close_reason = ''
            exit_price   = current_price

            # P1: Force exit < FORCE_EXIT_MINUTES (3 menit)
            if days_left is not None and 0 <= days_left < CFG['FORCE_EXIT_MINUTES'] / 1440:
                should_close = True
                close_reason = f'FORCE_EXIT (<{CFG["FORCE_EXIT_MINUTES"]}m sisa)'

            # P2: Market sudah expired
            elif days_left is not None and days_left < 0:
                should_close = True
                close_reason = 'EXPIRED'
                exit_price   = 0.0

            # P3: Time exit < TIME_EXIT_MINUTES (45 menit)
            elif days_left is not None and 0 <= days_left < CFG['TIME_EXIT_MINUTES'] / 1440:
                should_close = True
                close_reason = f'TIME_EXIT (<{CFG["TIME_EXIT_MINUTES"]}m sisa)'
...
# ══════════════════════════════════════════════════════════════════
# DISPLAY — Aman di Railway (no TTY, no clear)
# ══════════════════════════════════════════════════════════════════
def banner():
    mode = 'REAL TRADE' if AUTO_TRADE and PRIVATE_KEY else 'PAPER TRADE'
    print('=' * 70)
    print('  POLYMARKET AUTO BOT v11.1')
    print(f'  Mode: {mode} | '
          f'TP: +{CFG["TAKE_PROFIT_PCT"]:.0f}% | '
          f'SL: -{CFG["STOP_LOSS_PCT"]:.0f}% | '
          f'TimeExit: <{CFG["TIME_EXIT_MINUTES"]}m | '
          f'ForceExit: <{CFG["FORCE_EXIT_MINUTES"]}m | '
          f'MaxExp: ${CFG["MAX_EXPOSURE"]:.0f}')
    print('=' * 70)
...
def display_stats(st: dict, pm: 'PositionManager'):
    ...
    if st['recent']:
        ...
        for t in st['recent']:
            ...
            rows.append([ts[11:16], sig[:12], q,
                         f'{ep:.3f}', f'${amt:.2f}',
                         status, pnl_str, (reason or '')[:15]])
        print(tabulate(rows,
            headers=['Jam', 'Signal', 'Pasar', 'Entry', 'Bet', 'Status', 'P&L', 'Alasan'],
            tablefmt='simple'))
...
def display(results, stats_scan, stats_j, pm, closed_this_scan):
    ...
    print(f'  [{sp}] {ts} | Scan #{stats_scan["scans"]} | '
          ...
          f'{stats_scan["ms"]}ms | '
          f'Closed scan ini: {len(closed_this_scan)}')
    ...
    if closed_this_scan:
        ...
        for c in closed_this_scan:
            ...
            print(f'    [{status}] #{c["pos"]["id"]} | {c["reason"]} | '
                  f'P&L: ${pnl:+.3f} | {c["pos"]["question"][:40]}')
...
async def main():
    init_db()
    banner()
    log.info('=== POLYMARKET AUTO BOT v11.1 START ===')
    log.info(f'TIME_EXIT={CFG["TIME_EXIT_MINUTES"]}m | '
             f'FORCE_EXIT={CFG["FORCE_EXIT_MINUTES"]}m | '
             f'MAX_EXPOSURE=${CFG["MAX_EXPOSURE"]} | '
             f'MAX_POS={CFG["MAX_POSITIONS"]}')
    ...
    log.info(f'Loaded {len(already_opened)} open market IDs dari DB')

    connector = aiohttp.TCPConnector(limit=50, limit_per_host=15,
                                     ttl_dns_cache=300, ssl=False)
    hdrs = {'User-Agent': 'Mozilla/5.0 PolyBot/11.1', 'Accept': 'application/json'}

    async with aiohttp.ClientSession(connector=connector, headers=hdrs) as session:
        if TELEGRAM_TOKEN:
            mode = 'REAL TRADE' if AUTO_TRADE and PRIVATE_KEY else 'PAPER TRADE'
            await tg(session,
                f'<b>Polymarket Auto Bot v11.1 Online!</b>\n'
                f'Mode: <b>{mode}</b>\n'
                f'Max: {CFG["MAX_POSITIONS"]} posisi / ${CFG["MAX_EXPOSURE"]:.0f} exposure\n'
                f'Auto-open: STRONG BUY + ARBITRAGE\n'
                f'TP: +{CFG["TAKE_PROFIT_PCT"]:.0f}% | '
                f'SL: -{CFG["STOP_LOSS_PCT"]:.0f}%\n'
                f'TimeExit: <{CFG["TIME_EXIT_MINUTES"]}m | '
                f'ForceExit: <{CFG["FORCE_EXIT_MINUTES"]}m')
        ...
        while True:
            ...
            try:
                ...
                # ── 4. AUTO-OPEN (hanya is_auto=True) ───────────
                # Ambil kandidat terbaik berdasarkan skor
                auto_candidates = [
                    r for r in results
                    if r.get('is_auto')
                    and r['id'] not in already_opened
                    and r['liquidity'] >= CFG['MIN_LIQUIDITY']
                    and (r['days'] is None or
                         r['days'] >= (CFG['TIME_EXIT_MINUTES'] * 2) / 1440)
                ]
                ...
                if auto_candidates:
                    ...
                    if can:
                        ...
                        if pos_id:
                            ...
                            log.info(f'AUTO-OPEN: {best["signal"]} | '
                                     f'{best["question"][:50]} | '
                                     f'Skor={best["score"]:.0f}')
    ...
    final = db_get_stats()
    log.info(f'Bot berhenti. Scan={scans} | '
             f'Total={final["total"]} | Win={final["wins"]} | '
